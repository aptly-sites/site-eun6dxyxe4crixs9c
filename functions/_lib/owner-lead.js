// Shared logic behind /api/owner-lead and /api/forms/contact — ported from the source repo's
// artifacts/et-website-v1/server/owner-lead.mjs (a Vercel serverless function) to run as a
// Cloudflare Pages Function instead. All of this is runtime-agnostic (plain fetch calls, no
// Node-specific APIs), so the port is close to line-for-line; only the two callers
// (functions/api/owner-lead.js, functions/api/forms/contact.js) differ, since Pages Functions use
// the Fetch API's Request/Response instead of Vercel's Express-style (req, res).
...
const FORM_DETAILS = {
  ...
  'Eviction Cost Calculator – Consultation Request': {
    description:
      'The owner used the Eviction Cost Calculator and asked EquityTeam to contact them to schedule a conversation about property management.',
...
export function resolveMapping(schema, config, stageName, requestedSource = 'Website') {
  ...
  const field = (key, label, type) => {
    const found =
      schema.find(f => f.key === key && f.type === type) ||
      schema.find(f => f.label?.trim().toLowerCase() === label.toLowerCase() && f.type === type);
  ...
  if (!fields.find(f => f.uuid === result.stage)?.data?.some(o => o.label === stageName)) {
    throw new Error('Required board option missing');
  }
...
export function cardPayload(lead, mapping, contactId, stageName, relatedContactIds = []) {
  ...
  const notes = [
    ...
    lead.message
      ...
      : '',
    details.referral
      ? `<p><strong>Referring agent:</strong> ${escape(
          `${lead.referrerFirstName} ${lead.referrerLastName}`
        )}<br><strong>Agent email:</strong> ${escape(lead.referrerEmail)}<br><strong>Agent phone:</strong> ${escape(
          lead.referrerPhone
        )}</p>`
      : '',
...
export function reportEmailPayload(lead, cardId, env) {
  ...
  return {
    ...
    body: `<div style="font-family:Arial,sans-serif;color:#152a34;line-height:1.55;max-width:680px"><h1 style="font-size:26px">${escape(
      details.reportTitle
    )}</h1><p>Hi ${escape(lead.firstName)},</p><p>Here is the calculator report you requested for <strong>${escape(
      lead.address
    )}</strong>.</p><ul>${rows}</ul><p>${escape(details.disclaimer)}</p><p><a href="${escape(lead.pageUrl)}">${escape(
      details.returnLabel
    )}</a></p><p>EquityTeam Property Management<br><a href="https://www.equityteam.com">equityteam.com</a></p></div>`,
...
export async function createOwnerLead(lead, { env, fetchImpl = fetch } = {}) {
  const token = env?.APTLY_API_TOKEN,
    board = env?.APTLY_OWNER_LEADS_BOARD_ID,
    stage = env?.APTLY_OWNER_LEADS_STAGE;
  ...
  const card = await api(
    `board/${board}`,
    cardPayload(lead, mapping, contact._id, stage, referrerContactId ? [referrerContactId] : [])
  );
