...
const FORM_DETAILS = {
  'Free Rental Analysis': {
    description:'The owner completed the Free Rental Analysis request form to ask EquityTeam for a professional rent estimate for their property.',
    completeContact:true,
    verifiedAddress:true,
  },
  'For Sale – Free Rental Analysis': {
    description:'The visitor requested a free rental pricing analysis while browsing EquityTeam properties for sale.',
  },
  'Portal Login – Free Rental Analysis': {
    description:'The visitor requested a free rental pricing analysis from the EquityTeam Portal Logins page.',
  },
  'Rental Match Request': {
    description:'The prospective resident asked EquityTeam to contact them when a rental matching their preferences becomes available.',
  ...
  'PM Fee ROI Calculator – Email Report': {
    description:'The owner used the Property Management Fee ROI Calculator and requested an emailed copy of their personalized cost-benefit report.',
  ...
  '1031 Exchange Calculator – Consultation Request': {
    description:'The owner used the 1031 Exchange Calculator and asked EquityTeam to contact them to schedule a consultation about their investment property.',
  ...
  'STR vs. LTR Calculator – Email Report': {
    ...
    disclaimer:'This estimate is for informational purposes. Short-term rental rules vary by municipality, and this report is not tax or legal advice.',
...
export function normalizeLead(body) {
  ...
  const lead = Object.fromEntries(Object.entries({name:120,firstName:80,lastName:80,phone:40,email:254,address:400,placeId:300,message:2000,summary:8000,formSource:160,pageTitle:200,pageUrl:500,referrerFirstName:80,referrerLastName:80,referrerPhone:40,referrerEmail:254}).map(([key,max])=>[key,clean(body[key],max)]));
  ...
  if (!lead.name || !lead.address || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(lead.email)) throw new Error('Enter your name, email, and property address.');
  if (!lead.pageTitle || !/^https?:\/\//i.test(lead.pageUrl)) throw new Error('Invalid page context');
  if (details.completeContact && (!lead.firstName || !lead.lastName || !/^[+()\d\s.-]{7,40}$/.test(lead.phone))) throw new Error('Missing owner contact details');
  if (details.verifiedAddress && (!lead.firstName || !lead.lastName || !lead.placeId || !lead.summary)) throw new Error('Missing report details');
  if (details.consultation && (!lead.firstName || !lead.lastName || !/^[+()\d\s.-]{7,40}$/.test(lead.phone) || !lead.message || !lead.summary)) throw new Error('Missing consultation details');
  if (details.referral && (!lead.firstName || !lead.lastName || !/^[+()\d\s.-]{7,40}$/.test(lead.phone) || !lead.referrerFirstName || !lead.referrerLastName || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(lead.referrerEmail) || !/^[+()\d\s.-]{7,40}$/.test(lead.referrerPhone))) throw new Error('Missing referral details');
  return lead;
}

export async function verifyGoogleAddress(lead, {env=process.env,fetchImpl=fetch}={}) {
  ...
  const address=result.results?.find(item=>item.formatted_address && item.types?.some(type=>['street_address','premise','subpremise'].includes(type)));
...
export function resolveMapping(schema, config, stageName, requestedSource='Website') {
  ...
  const optionalField=(key,label,type)=>schema.find(f=>f.key===key && f.type===type)?.key || schema.find(f=>f.label?.trim().toLowerCase()===label.toLowerCase() && f.type===type)?.key;
  const result={owner:field('owner','Owner','person'),address:field('address','Rental Property Address','address'),description:field('description','Notes','rich-text'),stage:field('stage','Stage','stageselect'),source:field('leadSource','Source','sourceselect'),trackingPhone:optionalField('trackingPhone','Tracking Phone','phone') || optionalField('trackingPhone','Tracking Phone','tel'),trackingEmail:optionalField('trackingEmail','Tracking Email','email'),relatedContacts:optionalField('relatedContacts','Related Contacts','persons')};
...
export function cardPayload(lead, mapping, contactId, stageName, relatedContactIds=[]) {
  ...
  const notes=[
    ...
    lead.message ? `<p><strong>${details.referral ? 'Referral comments' : 'Owner message'}:</strong><br>${escape(lead.message).replace(/\n/g,'<br>')}</p>` : '',
    details.referral ? `<p><strong>Referring agent:</strong> ${escape(`${lead.referrerFirstName} ${lead.referrerLastName}`)}<br><strong>Agent email:</strong> ${escape(lead.referrerEmail)}<br><strong>Agent phone:</strong> ${escape(lead.referrerPhone)}</p>` : '',
    lead.summary ? `<p><strong>${summaryLabel}:</strong><br>${escape(lead.summary).replace(/\n/g,'<br>')}</p>` : '',
  ].filter(Boolean).join('');
  return {name:details.consultation || details.addressFallback ? `${lead.formSource} — ${lead.name}` : `${lead.formSource} — ${lead.address}`,[mapping.owner]:contactId,[mapping.address]:{formattedAddress:lead.address,address:lead.address},[mapping.description]:notes,[mapping.stage]:stageName,[mapping.source]:mapping.sourceValue || 'Website',...(mapping.relatedContacts && relatedContactIds.length?{[mapping.relatedContacts]:relatedContactIds}:{}),...(mapping.trackingPhone && lead.phone?{[mapping.trackingPhone]:lead.phone}:{}),...(mapping.trackingEmail?{[mapping.trackingEmail]:lead.email}:{})};
}
export function reportEmailPayload(lead, cardId, env=process.env) {
  const details=FORM_DETAILS[lead.formSource];
  ...
  const rows=lead.summary.split('\n').map(line=>`<li style="margin:0 0 8px">${escape(line)}</li>`).join('');
  return {
...
export async function createOwnerLead(lead, {env=process.env,fetchImpl=fetch}={}) {
  const token=env.APTLY_API_TOKEN, board=env.APTLY_OWNER_LEADS_BOARD_ID, stage=env.APTLY_OWNER_LEADS_STAGE;
  if(!token||!board||!stage) throw new Error('Lead integration is not configured');
  const api=async(path,body)=>{
    const response=await fetchImpl(`https://core-api.getaptly.com/api/${path}`,{method:body?'POST':'GET',headers:{'x-token':token,'Content-Type':'application/json'},...(body?{body:JSON.stringify(body)}:{}),signal:AbortSignal.timeout(15000)});
  ...
  const contactResult=await api('contacts',{firstname:lead.firstName || fallbackFirst,lastname:lead.lastName || fallbackLast.join(' '),email:lead.email,contactType:details.referral?'Owner Prospect':'Owner',...(lead.phone?{phone:[{number:lead.phone,type:'mobile'}]}:{})});
  ...
  if(details.referral) {
    const referrerResult=await api('contacts',{firstname:lead.referrerFirstName,lastname:lead.referrerLastName,email:lead.referrerEmail,contactType:'Agents',phone:[{number:lead.referrerPhone,type:'mobile'}]});
...
export default async function handler(req,res) {
  ...
  if(!/^application\/json(?:;|$)/i.test(req.headers['content-type'] || '')) return res.status(415).json({success:false,error:'JSON required'});
  if(req.headers.origin) { try { if(new URL(req.headers.origin).host!==req.headers.host) return res.status(403).json({success:false,error:'Invalid origin'}); } catch {return res.status(403).json({success:false,error:'Invalid origin'});} }
  let body=req.body;
  try { if(typeof body==='string') body=JSON.parse(body); if(JSON.stringify(body || {}).length>16000) return res.status(413).json({success:false,error:'Submission too large'}); } catch { return res.status(400).json({success:false,error:'Invalid submission'}); }
  if(clean(body?.website,200)) return res.status(200).json({success:true});
  let lead;
  try {lead=await verifyGoogleAddress(normalizeLead(body));} catch(error) {console.error('Owner lead validation failed:',error instanceof Error ? error.message : 'Unknown error');return res.status(400).json({success:false,error:'Please complete every required field with valid information.'});}
  // Short per-instance throttle; deploy behind the host's rate limiting as well.
  const now=Date.now(); for(const [key,time] of recent) if(now-time>60000) recent.delete(key);
  const key=lead.email.toLowerCase();
  if(recent.has(key)) return res.status(429).json({success:false,error:'Please wait a minute before submitting again.'});
  recent.set(key,now);
  try {await createOwnerLead(lead);return res.status(200).json({success:true});}
  catch(error) {console.error('Owner lead submission failed:',error instanceof Error ? error.message : 'Unknown error');return res.status(502).json({success:false,error:'We could not confirm your request. Please contact EquityTeam or try again later.'});}
}
